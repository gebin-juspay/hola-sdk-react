...
const { TestidSDKReact } = NativeModules;

export default TestidSDKReact as HyperSdkReactType;
```
...
```ts
TestidSDKReact.preFetch(JSON.stringify(preFetchPayload));
```
...
```ts
TestidSDKReact.createHyperServices();
```
...
```ts
TestidSDKReact.initiate(JSON.stringify(initiatePayload));
```
...
```ts
TestidSDKReact.process(JSON.stringify(processPayload));
```
...
```ts
TestidSDKReact.processWithActivity(JSON.stringify(processPayload));
```

### Step-4: Listen to events from HyperSDK

`Hyper SDK` Native Module will be emitting all the relevant events to JS via `RCTDeviceEventEmitter` and JavaScript modules can then register to receive events by invoking `addListener` on the `NativeEventEmitter` class in the `componentDidMount()` method with the event name `'HyperEvent'` (You can use the `TestidSDKReact.HyperEvent` as well). The listener will return a `stringified JSON` response (`resp`).
...
```ts
 componentDidMount() {
   ...
   const eventEmitter = new NativeEventEmitter(NativeModules.TestidSDKReact);
   this.eventListener = eventEmitter.addListener(TestidSDKReact.HyperEvent, (resp) => {
...
If the blocking asynchronous call `TestidSDKReact.onBackPressed()` returns true, `Hyper SDK` will handle the back press, else merchant can handle it.

**Note**: `TestidSDKReact.isNull()` (refer [here](#helper-is-null)) can also be called before calling `onBackPressed()` to ensure that the HyperServices object is not null.

```ts
 componentDidMount() {
   ...
   BackHandler.addEventListener('hardwareBackPress', () => {
     return !TestidSDKReact.isNull() && TestidSDKReact.onBackPressed();
   });
   ...
 }
...
```ts
var isNull: boolean = TestidSDKReact.isNull();
...
```ts
TestidSDKReact.isInitialised().then((init: boolean) => {
...
This is an optional method to update the base view controller in case if any new view controller is presented over top view controller after the SDK initiation. This method should be called before making `TestidSDKReact.process()` call.
...
```ts
TestidSDKReact.notifyAboutRegisterComponent(TestidSDKReact.JuspayHeaderAttached)
AppRegistry.registerComponent(TestidSDKReact.JuspayHeaderAttached, () => CustomComponent);
```
...
Therefore the component must fire `TestidSDKReact.updateMerchantViewHeight(<section_name>, <height>);`

For example
```ts
TestidSDKReact.updateMerchantViewHeight(TestidSDKReact.JuspayHeader, 200);
```
...
```ts
  onLayout={(event) => {
        ...
        TestidSDKReact.updateMerchantViewHeight(TestidSDKReact.JuspayHeader, height);
      }}
...
hola-sdk-react is distributed under [AGPL-3.0-only](https://github.com/juspay/hola-sdk-react/src/main/LICENSE.md) license.
