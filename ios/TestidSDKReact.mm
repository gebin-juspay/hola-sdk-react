...
#import <TestidPaymentsServices/TestidPaymentsServices.h>

__weak static TestidPaymentsServices *_hyperServicesReference;
...
RCT_EXPORT_METHOD(preFetch:(NSString *)data) {
    if (data && data.length>0) {
        @try {
            NSDictionary *jsonData = [TestidSDKReact stringToDictionary:data];
            if (jsonData && [jsonData isKindOfClass:[NSDictionary class]] && jsonData.allKeys.count>0) {
                [TestidPaymentsServices preFetch:jsonData];
            } else {
...
RCT_EXPORT_METHOD(createHyperServices) {
    if (self.hyperInstance == NULL) {
        self.hyperInstance = [[TestidPaymentServices alloc] init];
    ...
    if (data && data.length>0) {
        @try {
            ...
            if (jsonData && [jsonData isKindOfClass:[NSDictionary class]] && jsonData.allKeys.count>0) {
                ...
                __weak TestidSDKReact *weakSelf = self;
...
{
    TestidPaymentsServices *hyperServicesInstance = _hyperServicesReference;
